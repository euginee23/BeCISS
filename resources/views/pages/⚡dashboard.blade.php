    ...
    @elseif(auth()->user()->isResident())
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <flux:text class="text-zinc-600 dark:text-zinc-300 mt-1">
                            Your account is not yet linked to a resident profile. Please visit the barangay hall or contact staff to have your account linked to your resident record.
                        </flux:text>
                    </div>
                </div>
            </div>
        @else
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 flex flex-col gap-3">
                    <div class="flex items-center justify-between">
                        <flux:text class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Pending Certificates</flux:text>
                        <div class="size-9 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center">
                            <flux:icon name="document-text" class="size-4 text-amber-600 dark:text-amber-400" />
                        </div>
                    </div>
                    <flux:heading size="2xl" class="text-zinc-900 dark:text-white">{{ number_format($this->pendingCertificates) }}</flux:heading>
                    <flux:text class="text-xs text-zinc-400">Awaiting processing</flux:text>
                </div>
        ...
        <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 flex flex-col">
            <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-700">
                <flux:heading>My Recent Certificates</flux:heading>
                <flux:button :href="route('resident.certificates.index')" variant="ghost" size="sm" wire:navigate>View all</flux:button>
            </div>
            ...
            @if($this->recentCertificates->isEmpty())
                <div class="flex items-center justify-center py-12 text-zinc-400">
                    <div class="text-center">
                        ...
                        <flux:text>No certificate requests yet</flux:text>
                        <div class="mt-3">
                            <flux:button :href="route('resident.certificates.index')" variant="primary" size="sm" wire:navigate>Request a Certificate</flux:button>
                        </div>
                    </div>
                </div>
            @else
        ...
        <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 flex flex-col">
            <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-700">
                <flux:heading>My Upcoming Appointments</flux:heading>
                <flux:button :href="route('resident.appointments.index')" variant="ghost" size="sm" wire:navigate>View all</flux:button>
            </div>
            ...
            @if($this->upcomingAppointments->isEmpty())
                <div class="flex items-center justify-center py-12 text-zinc-400">
                    <div class="text-center">
                        ...
                        <flux:text>No upcoming appointments</flux:text>
                        <div class="mt-3">
                            <flux:button :href="route('resident.appointments.index')" variant="primary" size="sm" wire:navigate>Book an Appointment</flux:button>
                        </div>
                    </div>
                </div>
            @else
