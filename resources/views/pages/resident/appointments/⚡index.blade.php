...
<div class="flex flex-col gap-6">
    ...
    <div class="flex gap-1 border-b border-zinc-200 dark:border-zinc-700">
        <button
            ...
            class="px-4 py-2.5 text-sm font-medium border-b-2 transition-colors {{ $tab === 'upcoming' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-200' }}"
        >Upcoming</button>
        <button
            wire:click="$set('tab', 'past')"
            class="px-4 py-2.5 text-sm font-medium border-b-2 transition-colors {{ $tab === 'past' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-200' }}"
        >Past</button>
    </div>
    ...
    @if($this->appointments && $this->appointments->isNotEmpty())
        <div class="flex flex-col gap-4">
            @foreach($this->appointments as $appt)
                <div wire:key="{{ $appt->id }}" class="group relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 transition-all hover:shadow-lg hover:border-emerald-300 dark:hover:border-emerald-700">
                    <div class="absolute inset-0 bg-gradient-to-br from-teal-50 to-transparent dark:from-teal-950/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <div class="relative flex items-start justify-between gap-4">
                        <div class="flex gap-3 flex-1 min-w-0">
                            <div class="size-10 rounded-xl bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center shrink-0 mt-0.5">
                                <flux:icon name="calendar" class="size-5 text-teal-600 dark:text-teal-400" />
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                @if($appt->description)
                                    <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3 line-clamp-2">{{ $appt->description }}</p>
                                @endif
                                ...
                                @if($appt->cancellation_reason)
                                    <div class="mt-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2 text-sm text-red-600 dark:text-red-400">
